...
class _EventRegistrationScreenState extends State<EventRegistrationScreen> {
  ...
  Future<void> _submitRegistration() async {
    ...
    try {
      ...
      if (result['success'] == true) {
        final registerId = result['registerId'] as String?;
      
      if (registerId == null || registerId.isEmpty) {
        throw Exception('Registration ID is null or empty');
      }

      print('Registration successfull ID: $registerId');

      // Navigate to payment screen for paid events
      if (!widget.event.isFree) {
        print('Navigating to payment screen for paid event');
        ...
        // Return success to event details screen
        Navigator.of(context).pop(true);
        
      } else {
        // For free events, show success directly
        print('Free event - showing success dialog');
        _showSuccessDialog(context, false, registerId); // false = free event
      }
    } else {
      String errorMessage = result['error']?.toString() ?? 'Unknown error';
  ...
  void debugRegistrationFlow() {
    print('=== DEBUG REGISTRATION FLOW ===');
    print('1. Event: ${widget.event.name} (ID: ${widget.event.id})');
    print('2. Event is free: ${widget.event.isFree}');
    print('3. Current user: ${_currentUser?.id ?? "Not logged in"}');
    print('4. Form data:');
    print('   - Name: ${_fullNameController.text}');
    print('   - Email: ${_emailController.text}');
    print('   - Phone: ${_phoneController.text}');
    print('5. Ticket quantity: ${widget.ticketQuantity}');
  }
}